...
class DecisionEngine:
    ...
    def evaluate_compatibility(self, media: MediaInfo) -> CompatibilityDetails:
        ...
        subtitle_ok, subtitle_reasons = FFprobeAnalyzer.mp4_subtitle_compatible(
            media,
            allow_dropping_image_subtitles=self.config.subtitles.drop_incompatible_image_subtitles,
        )
        ...
        if comp.dv_present:
            ...
            if not safe_profile or (container_switch and not self.config.dolby_vision.remux_dv_from_mkv_to_mp4_is_safe):
                ...
                if self._can_use_dovi_muxer(media, comp, dv_caps):
                    ...
                    if any(stream.is_image_subtitle for stream in media.subtitle_streams):
                        if self.config.subtitles.ocr_image_subtitles:
                            reasons.append("OCR image subtitle tracks to mov_text for Apple-native MP4 output")
                        elif self.config.subtitles.drop_incompatible_image_subtitles:
                            reasons.append("Dropping incompatible image subtitle tracks for Apple-native MP4 output")
