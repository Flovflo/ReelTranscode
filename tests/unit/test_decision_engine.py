...
def test_dv_fragile_uses_dovi_muxer_and_drops_image_subtitles_by_default(tmp_path: Path):
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    for name in ["DoViMuxer", "MP4Box", "mediainfo", "mp4muxer", "ffmpeg", "ffmpeg_dovi_compat"]:
        path = bin_dir / name
    ...
    assert decision.strategy == Strategy.REMUX_ONLY
    assert decision.use_dovi_muxer is True
    assert any("Dropping incompatible image subtitle tracks" in reason for reason in decision.reasons)


def test_dv_fragile_can_ocr_image_subtitles_and_keep_dovi_muxer(tmp_path: Path):
    bin_dir = tmp_path / "bin"
    bin_dir.mkdir()
    for name in ["DoViMuxer", "MP4Box", "mediainfo", "mp4muxer", "ffmpeg", "ffmpeg_dovi_compat"]:
        path = bin_dir / name
    ...
    assert decision.strategy == Strategy.REMUX_ONLY
    assert decision.use_dovi_muxer is True
    assert any("OCR image subtitle tracks" in reason for reason in decision.reasons)
