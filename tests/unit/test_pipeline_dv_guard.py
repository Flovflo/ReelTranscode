...
class _FakePlanner:
    ...
    def build(self, _media, decision, _compatibility, _source_root):
        return ExecutionPlan(
            ...
            cleanup_dirs=list(self.cleanup_dirs),
            notes=[],
        )
...
def test_pipeline_removes_dv_workspace_after_success(tmp_path: Path):
    source = tmp_path / "watch" / "movie.mkv"
    source.parent.mkdir(parents=True)
    source.write_bytes(b"source")
    target = tmp_path / "optimized" / "movie.mp4"
    workspace = tmp_path / "tmp" / ".movie.1234567890.dovi"
    workspace.mkdir(parents=True)
    temp = workspace / "movie.tmp.mp4"
    ...
    processor.engine = _FakeEngine(decision, compatibility)
    processor.planner = _FakePlanner(
        source,
        target,
        temp,
